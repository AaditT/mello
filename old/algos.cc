/* algos.cc - Advanced machine learning algorithms for Mello.
 * Written by Srihari Nanniyur.
 * You are now about to witness my brilliance. Prepare yourself.
 */
...
std::vector<std::string> melody(std::string key, std::string scale, int num)
{
    ...
    else if (scale == "pentatonic major") notes = gen.notes({2, 2, 3, 2});
    else if (scale == "pentatonic minor") notes = gen.notes({3, 2, 2, 3});
    else if (scale == "blues")            notes = gen.notes({3, 2, 1, 1, 3});
    else throw std::runtime_error("Invalid key signature.");

    for (const auto &e : notes) { std::cout << e << std::endl; }
